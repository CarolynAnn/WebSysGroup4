...
function edit_user($firstName, $lastName, $email, $password){
    ...
    $dbcon->exec("UPDATE `users` SET `firstName` = '$firstName', `lastName` = '$lastName', `email` = '$email', `password` = '$salted', `salt`= '$salt' WHERE `userID` = " . $_SESSION['userID']);
...
function validPassword($password){
    ...
    // Query `users` database for this user's salt, get's set to empty string by default
    $query = $dbcon->query("SELECT `salt` FROM `users` WHERE `userID`=" . $_SESSION['userID'] . ";");
    $query = $query->fetch(PDO::FETCH_ASSOC);
    $salt = ($query) ? $query['salt'] : '';
    ...
    // Query `users` database for entry with entered email and salted password
    $query = $dbcon->query("SELECT * FROM `users` WHERE `userID`= " . $_SESSION['userID'] . " AND `password`='$salted';");
    if($query->rowCount() > 0) {
